...
function Details() {
  ...
  useEffect(() => {
    ...
    const getCredits = async () => {
      ...
      const promises = credits.cast.map(creditItem => {
        return request.getCreditDetails(creditItem.credit_id)
      })
      Promise.all(promises).then(results => {
        setCreditsDetails(results)
      })
    }
                      ...
                      <ul className='cast_list'>
                        {credits &&
                          credits.cast.slice(0, 10).map((item, index) => {
                            return (
                              <li key={item.id} className='cast_item'>
                                ...
                                {creditsDetails &&
                                  creditsDetails[index].media &&
                                  creditsDetails[index].media.seasons && (
                                    <span className='cast_count_episodes'>
                                      {creditsDetails[
                                        index
                                      ].media.seasons.reduce((a, b) => {
                      ...
                      <div className='season section'>
                        ...
                        <div className='season_item'>
                          <div className='season_img'>
                            <Link to={`season`}>
                              <LazyLoadImage
                                src={`${IMAGE_PATH}${seasons[currentSeason].poster_path}`}
                                effect='opacity'
                                alt=''
                              />
                            </Link>
                    ...
                    <div className='section'>
                      ...
                      {data.networks && data.networks.length > 0 && (
                        <div className='panel'>
                          ...
                          <img
                            src={`${IMAGE_PATH}${data.networks[0].logo_path}`}
                            alt=''
                          />
                        </div>
                      )}
